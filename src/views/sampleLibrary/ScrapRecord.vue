...
<!--            <a-space align="center">-->
<!--              <span style="font-size: 14px; margin-right: 9px ">工厂仓库:</span>-->
...
<!--              <a-select-option v-for="data in option.inventory_location" :key="data.value">-->
<!--                {{ data.label }}-->
<!--              </a-select-option>-->
<!--            </a-select>-->
<!--          </a-space>-->
      ...
      <div class="table-frame">
        ...
        >
          <a-space slot="operate" slot-scope="text, operate, i" class="operate-row">
            <a-button type="link" @click="openRepair(operate, i)">出库</a-button>
          </a-space>
        </a-table>
        <!--    分页-->
...
<script>
  import {getMouldOptions,getScrapRecord} from '@/api/api'
  export default {
    name: 'ScrapRecord',
    mounted() {
    ...
    data() {
      return{
        ...
        currentPage: 1, // 当前页
        columns: [
          { title: '报废时间', dataIndex: 'createTime', key: 'createTime', align: 'center', },
          ...
          { title: '报废数量', dataIndex: 'outWarehouse', key: 'outWarehouse', align: 'center', },
    ...
    methods: {
      ...
      getList(){
        ...
        getScrapRecord(this.searchParams).then((data)=>{
          data = data.result;
