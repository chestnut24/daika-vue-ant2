    ...
    <div class="information-frame" v-for="(item,index) in dataSource" :key="index">
      <div class="information-item-top">
        ...
        <div class="item-top-right">
          ...
          <span style="font-size: 14px;color: #B0B0B0"  @click="goGetSample(item)"><a-icon type="right" /></span>
        </div>
      </div>
      ...
      <div class="information-item-down">
        <div class="information">
          <span style="color:rgba(0,0,0,0.45);font-size: 14px;">待发库存</span>
          <span style="color: #347EF2;font-size: 18px;"><b>{{item.needSendNum}}</b></span>
        </div>
        <div class="information">
          <span style="color:rgba(0,0,0,0.45);font-size: 14px;">待发需求</span>
          <span style="color: #347EF2;font-size: 18px;"><b>{{item.needSendNumDemand}}</b></span>
        </div>
        <div class="information">
          <span style="color:rgba(0,0,0,0.45);font-size: 14px;">已提货</span>
          <span style="color: #347EF2;font-size: 18px;"><b>{{item.outWarehouseNum}}</b></span>
        </div>
      </div>
...
<script>
  import {getDelivery, getMouldOptions} from '@/api/api'
  export default {
    name: 'sampleGetSample',
    mounted() {
      this.getDic();
      setTimeout(()=>{
        this.getList();
    ...
    methods:{
      ...
      getList(){
        getDelivery(this.searchParams).then((data)=>{
          ...
          this.dataSource.forEach((item,index)=>{
            this.dataSource[index].batchArr = [];
            console.log('item',item.batch.split(','));
            item.batch.split(',').forEach((item2)=>{
              if(item2!=''){
                this.dataSource[index].batchArr.push(item2);
              }
            })
          });
        })
      ...
      goGetSample(item){
        ...
        this.$router.push({
          path:'/app/sampleWaitingBordGetSample',
          query:{
            id:item.id,
            projectNo:item.projectNo,
            purpose:item.purpose,
            projectManager:item.projectManager,
            shippingFlowSheet:item.shippingFlowSheet
          }
        })
      }
