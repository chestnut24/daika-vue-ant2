<template>
  <a-row :gutter="10">
          ...
          <span slot="action" slot-scope="text, record">
            ...
            <a-dropdown>
              ...
              <a-menu slot="overlay">
                ...
                <a-menu-item>
                  <a @click="handleSelect(record)">配置</a>
                </a-menu-item>
                <a-menu-item>
    ...
    <a-modal v-model="visible1" title="选择试制地与工序" :width="800" okText="保存" @ok="saveModal">
      <div>
        <span>
          试制地：
        </span>
        <a-select style="width: 500px" mode="multiple" v-model="selectedSiteIdList">
          <a-select-option :value="opt.id" v-for="opt in option.trial_production_site" :key="opt.id">
            {{opt.label}}
          </a-select-option>
        </a-select>
      </div>
      <div style="margin-top: 20px">
        ...
        <a-select style="width: 500px" mode="multiple" v-model="selectedTechnologyIdList">
          <a-select-option :value="opt.id" v-for="opt in processList" :key="opt.id">
            {{opt.name}}
          </a-select-option>
        </a-select>
      </div>
...
<script>
  ...
  import { getOptions, getProcessTreeTwo, getRoleSiteAndTechnology, editRoleSiteAndTechnology } from '@/api/api'
  ...
  export default {
    ...
    computed: {
      ...
      mounted(){
          getOptions('trial_production_site', '').then((data) => {
              this.option.trial_production_site = data;
          });
          getProcessTreeTwo().then(res => {
              console.log('ews',res.result)
    ...
    methods: {
      ...
      handleSelect(record) {
        ...
        this.visible1 = true;
          getRoleSiteAndTechnology({roleId: record.id}).then(res => {
              this.selectedSiteIdList = res.result.siteIdList;
              this.selectedTechnologyIdList = res.result.technologyIdList;
          })
      },
        saveModal() {
          console.log({siteIdList: this.selectedSiteIdList ,technologyIdList: this.selectedTechnologyIdList})
            editRoleSiteAndTechnology({roleId: this.currentRecord.id, siteIdList: this.selectedSiteIdList ,technologyIdList: this.selectedTechnologyIdList}).then(res => {
                if (res.success) {
