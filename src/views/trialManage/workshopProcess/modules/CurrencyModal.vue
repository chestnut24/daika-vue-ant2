        ...
        <a-table
          ...
          <template slot="currency" slot-scope="text, record">
            <template v-if="record.trialProductionTaskPlanCommonList?record.trialProductionTaskPlanCommonList.length>0:false">
              <span v-if="record.trialProductionTaskPlanCommonList.find(ite=>{return ite.planType==record.type && ite.planStatus==1}) ">已策划</span>
              <span v-else-if="record.trialProductionTaskPlanCommonList.find(ite=>{return ite.planType==record.type && ite.planStatus==0}) ">未策划</span>
            </template>
        ...
        data() {
            return {
                ...
                columns: [ // 表头
                    ...
                    { title: '通用', dataIndex: 'currency', align: 'center',scopedSlots: { customRender: 'currency' } },
                ],
            }
        },
        created() {
            ...
            add(record,tag,name){
               this.columns.forEach((item,index)=>{
                   if(item.dataIndex=='currency'){
                       Vue.set(this.columns[index],'title',record.title)
                   }
               })
                this.record=record
                this.name=name
