        ...
        <a-table
          ...
          <template slot="archives" slot-scope="text, record">
            <template v-if="record.trialProductionTaskPlanCommonList?record.trialProductionTaskPlanCommonList.length>0:false">
              <span v-if="record.trialProductionTaskPlanCommonList.find(ite=>{return ite.planType=='试制档案' && ite.planStatus==1}) ">已策划</span>
              <span v-else-if="record.trialProductionTaskPlanCommonList.find(ite=>{return ite.planType=='试制档案' && ite.planStatus==0}) ">未策划</span>
              <span v-else-if="record.trialProductionTaskPlanCommonList.find(ite=>{return ite.planType!=='试制档案' && ite.planStatus!==0 && ite.planStatus==1}) ">未策划</span>
            </template>
          ...
          <template slot="track" slot-scope="text, record">
            <template v-if="record.trialProductionTaskPlanCommonList?record.trialProductionTaskPlanCommonList.length>0:false">
                <span v-if="record.trialProductionTaskPlanCommonList.find(ite=>{return ite.planType=='试制跟踪' && ite.planStatus==1}) ">已策划</span>
                <span v-else-if="record.trialProductionTaskPlanCommonList.find(ite=>{return ite.planType=='试制跟踪' && ite.planStatus==0}) ">未策划</span>
                <span v-else-if="record.trialProductionTaskPlanCommonList.find(ite=>{return ite.planType!=='试制跟踪' && ite.planStatus!==0 && ite.planStatus==1}) ">未策划</span>
            </template>
            <span v-else>未策划</span>
          </template>
        </a-table>
        ...
        data() {
            return {
                ...
                columns: [ // 表头
                    ...
                    { title: '试制档案', dataIndex: 'requirement', align: 'center',scopedSlots: { customRender: 'archives' } },
                    { title: '试制跟踪', dataIndex: 'balance', align: 'center',scopedSlots: { customRender: 'track' } },
                ],
            }
        },
        created() {
